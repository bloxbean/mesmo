...
A living, categorized backlog of work for Mesmo. Mesmo compiles
...
- [ ] `P2` Split the monolithic Go `wrappers/go/mesmo/mesmo.go` (~2k LOC) and Rust `wrappers/rust/src/lib.rs` into focused modules for maintainability.
- [ ] `P2` Cross-wrapper error-handling review for consistent `MesmoError` semantics (codes, messages, idiomatic types).
...
- [x] `P2` ~~Expose **stake-key signing**~~ **Done** — added multi-role signing (historically `mesmo_account_sign_tx_multi`; today the typed role mask on `mesmo_account_sign_tx_handle`), which signs with any subset of `payment` / `stake` / `drep` / `committee_cold` / `committee_hot` (CCL's `Account.signWith*Key`), wired through all four wrappers (`sign_tx_with_keys` / `SignTxWithKeys` / `signTxWithKeys`). Fixes the `MissingVKeyWitnessesUTXOW` rejection for stake/vote/DRep certs; the original `mesmo_account_sign_tx` (payment only) is unchanged.
...
- [ ] `P1` Publish wrappers to registries: PyPI (`mesmo`), crates.io (`mesmo`), npm (`@bloxbean/mesmo`), and tag the Go module for the proxy. _(Renamed from `ccl` in PR #24. Python wheel (#15) and npm (#16) publish workflows are staged; crates.io + Go-tag flows still to write. A first release also unblocks the deferred download-path E2E tests — Go/Rust/musl currently seed the lib in CI because no release exists yet.)_
...
- [ ] `P2` **Runtime lib↔wrapper version check.** A native lib a version behind its wrapper fails confusingly; have each wrapper call `mesmo_version` on init and error clearly on mismatch.
...
- [~] `P1` **Evaluator abstraction + examples (pick-and-choose).** Give users a clear, per-language
  ...
  **Scalus** is the offline default baked into `libmesmo` (`ScalusTransactionEvaluator`), and a
...
`mesmo_quicktx_build` is offline by design: the caller supplies **UTXOs**, **protocol parameters**,
...
- [ ] `P2` **Reconcile the WISHLIST vs Non-Goals tension.** Satya's wishlist wants provider-fetched
  ...
  into the native `libmesmo` is not.* The Non-Goals note now says this explicitly.
...
- [~] `P1` Add a cross-wrapper parity test matrix asserting every `@CEntryPoint` is covered in every language. **Done (binding parity):** [`scripts/check_entrypoint_parity.py`](scripts/check_entrypoint_parity.py) — CI-guarded (a fast `entrypoint-parity` job, no native build) — extracts the canonical `mesmo_*` `@CEntryPoint` set from core and asserts each wrapper (Python/JS/Rust/Go) binds *exactly* that set, so a new/renamed entry point can't silently be missing from one language. _Remaining: test-coverage parity (assert each entry point is actually **exercised by a test**), which pairs with the Go/Rust test-breadth item above._
...
Surfaced by scanning upstream CCL. Mesmo now targets **0.8.0-pre5**, so all of these are
...
### Now available on CCL 0.8.0-pre5 (pinned since pre4)

- [x] `P1` ~~**Upgrade CCL 0.7.2 → 0.8.0**~~ **Done** — Mesmo is on `0.8.0-pre5` (the TxPlan refactor). The QuickTx wrapper was rewritten to TxPlan YAML; the 0.8.0 unified `Tx`/`ScriptTx` + `DepositMode` are exercised by the intent E2E suite. Re-pin to the stable `0.8.0` when it releases.
...
- [ ] `P1` **Typed errors instead of integer codes.** Every wrapper surfaces failures as an int code
      ...
      sentinel errors that work with `errors.Is`/`errors.As` (Go — note `mesmo.ErrInvalidAddress` is an
      ...
      `Account.Create(mesmo.Success)` compiling (untyped constants convert to `Network` today).
...
- [ ] `P2` **Go: 37 exported methods have no doc comment**, so `pkg.go.dev` renders a wall of bare
      ...
      documented only in the Java source). Naming also needs a pass: `mesmo.MesmoError` stutters,
...
- [ ] `P3` **Stale example/README instructions.** Go's example headers tell users to set
      `DYLD_LIBRARY_PATH`/`LD_LIBRARY_PATH`, which the loader never consults (it wants `MESMO_LIB_PATH`);
      Rust's README `build(...)` snippet has the wrong arity and does not compile; Python's README
      quick-start and 3 of 4 examples import from the private `mesmo._ffi` rather than `mesmo`.
...
- **Backend / HTTP provider modules *in the native `libmesmo`*** (Blockfrost, Koios, Ogmios) —
  ...
  The line is: convenience in wrapper code = yes; a provider baked into `libmesmo` = no.
