...
`libmesmo.{dylib,so,dll}` for your platform is resolved automatically, in order:

1. **`MESMO_LIB_PATH`** — a directory or the library file, to supply your own build;
2. a **per-version cache** (`os.UserCacheDir()/mesmo/<version>/`);
...
> So an account created with `mesmo.Mainnet` has an `AddressInfo.NetworkID` of `1` — `NetworkID` on
> the *returned* `AddressInfo` is the genuine on-chain value decoded from the address. Always pass
> a `mesmo.Network` constant; passing a raw int for the network is a compile error.
