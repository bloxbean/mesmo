...
// MesmoError represents an error from the CCL native library.
...
func (b *Mesmo) invoke(call func() int32) (string, error) {
	...
	if rerr := b.run(func() {
		if rc := call(); rc != Success {
			err = &MesmoError{Code: int(rc), Message: b.getError()}
		} else {
			s = b.getResult()
		}
	}); rerr != nil {
...
// expectedLibVersion is the native libmesmo version this wrapper expects, kept in lockstep with the
...
func (b *Mesmo) checkVersion() error {
	...
	if baseVersion(libVer) != baseVersion(expectedLibVersion) {
		return fmt.Errorf("libmesmo version %q is incompatible with the cardano-client-lib Go wrapper "+
			"(expects %q); the native library and wrapper must be the same version — update the module "+
			"or set MESMO_LIB_PATH/MESMO_LIB_VERSION to a matching libmesmo (set MESMO_SKIP_VERSION_CHECK=1 to bypass)",
