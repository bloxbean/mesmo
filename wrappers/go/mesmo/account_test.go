...
// assertMesmoError fails unless err is a non-nil *MesmoError. Used by the offline negative/error tests
// ported from the Python wrapper (which assert a MesmoError is raised).
func assertMesmoError(t *testing.T, op string, err error) {
	t.Helper()
	if err == nil {
	...
	var ce *MesmoError
	if !errors.As(err, &ce) {
		t.Fatalf("%s: expected *MesmoError, got %T: %v", op, err, err)
	}
}
...
func TestAccountSignTxInvalidCbor(t *testing.T) {
	...
	assertMesmoError(t, "SignTx(invalid cbor)", err)
}
