...
// FFI binding layer. The native library (libmesmo) is loaded with purego — pure-Go dlopen/dlsym, no
...
// One function variable per libmesmo entry point. purego marshals Go strings to C strings for the
...
// ensureLoaded resolves + dlopens libmesmo and binds every entry point, exactly once per process.
func ensureLoaded() error {
	loadOnce.Do(func() {
		...
		reg(&cclGetResult, "mesmo_get_result")
		reg(&cclGetLastError, "mesmo_get_last_error")
		reg(&cclFreeString, "mesmo_free_string")

		reg(&cclVersion, "mesmo_version")

		reg(&cclAddressInfo, "mesmo_address_info")
		reg(&cclAddressValidate, "mesmo_address_validate")
		reg(&cclAddressToBytes, "mesmo_address_to_bytes")
		reg(&cclAddressFromBytes, "mesmo_address_from_bytes")

		reg(&cclCryptoBlake2b256, "mesmo_crypto_blake2b_256")
		reg(&cclCryptoBlake2b224, "mesmo_crypto_blake2b_224")
		reg(&cclCryptoGenerateMnemon, "mesmo_crypto_generate_mnemonic")
		reg(&cclCryptoValidateMnemon, "mesmo_crypto_validate_mnemonic")
		reg(&cclCryptoSign, "mesmo_crypto_sign")
		reg(&cclCryptoVerify, "mesmo_crypto_verify")
		reg(&cclCryptoDeriveKey, "mesmo_crypto_derive_key")

		reg(&cclTxHash, "mesmo_tx_hash")
		reg(&cclTxSignSecretKey, "mesmo_tx_sign_with_secret_key")
		reg(&cclTxToJSON, "mesmo_tx_to_json")
		reg(&cclTxFromJSON, "mesmo_tx_from_json")
		reg(&cclTxDeserialize, "mesmo_tx_deserialize")

		reg(&cclPlutusDataHash, "mesmo_plutus_data_hash")
		reg(&cclPlutusDataToJSON, "mesmo_plutus_data_to_json")
		reg(&cclPlutusDataFromJSON, "mesmo_plutus_data_from_json")

		reg(&cclScriptNativeFromJSON, "mesmo_script_native_from_json")
		reg(&cclScriptHash, "mesmo_script_hash")

		reg(&cclQuicktxBuild, "mesmo_quicktx_build")
		reg(&cclAccountOpenMnemonic, "mesmo_account_open_mnemonic")
		reg(&cclAccountGetInfo, "mesmo_account_get_info")
		reg(&cclAccountSignTxHandle, "mesmo_account_sign_tx_handle")
		reg(&cclAccountCloseHandle, "mesmo_account_close")
		reg(&cclAccountCreateHandle, "mesmo_account_create_handle")
		reg(&cclAccountExportRecoveryPhrase, "mesmo_account_export_recovery_phrase")
	})
	return loadErr
}
