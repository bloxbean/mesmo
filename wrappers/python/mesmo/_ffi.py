...
class Mesmo:
    ...
    # Error codes
    MESMO_SUCCESS = 0
    MESMO_ERROR_GENERAL = -1
    MESMO_ERROR_INVALID_ARGUMENT = -2
    MESMO_ERROR_SERIALIZATION = -3
    MESMO_ERROR_CRYPTO = -4
    MESMO_ERROR_INVALID_NETWORK = -5
    MESMO_ERROR_INVALID_MNEMONIC = -6
    MESMO_ERROR_INVALID_ADDRESS = -7
    MESMO_ERROR_INSUFFICIENT_FUNDS = -8
    MESMO_ERROR_INVALID_TRANSACTION = -9
    # Raised for a malformed TxPlan — the most common failure on the core build path.
    MESMO_ERROR_TX_BUILD = -10
    ...
    def _resolve_lib_file(cls, lib_path=None):
        ...
        3. the copy bundled inside this package (``mesmo/_libs/``) — how an installed wheel ships it;
        ...
        env = os.environ.get('MESMO_LIB_PATH')
        if env:
    ...
    def __init__(self, lib_path=None):
        ...
        # DLL dependencies next to libmesmo.dll (no-op / absent on Unix).
        ...
        try:
            ...
            raise OSError(
                ...
                f"Install a platform wheel that bundles it, or set MESMO_LIB_PATH to the "
        ...
        from mesmo.accounts import Accounts
    ...
    def _setup_functions(self):
        ...
        lib.mesmo_version.argtypes = [c_void_p]
        lib.mesmo_version.restype = c_int

        lib.mesmo_get_result.argtypes = [c_void_p]
        lib.mesmo_get_result.restype = c_void_p

        lib.mesmo_get_last_error.argtypes = [c_void_p]
        lib.mesmo_get_last_error.restype = c_void_p

        lib.mesmo_free_string.argtypes = [c_void_p, c_void_p]
        lib.mesmo_free_string.restype = None

        # Address API
        lib.mesmo_address_info.argtypes = [c_void_p, c_char_p]
        lib.mesmo_address_info.restype = c_int

        lib.mesmo_address_to_bytes.argtypes = [c_void_p, c_char_p]
        lib.mesmo_address_to_bytes.restype = c_int

        lib.mesmo_address_from_bytes.argtypes = [c_void_p, c_char_p]
        lib.mesmo_address_from_bytes.restype = c_int

        lib.mesmo_address_validate.argtypes = [c_void_p, c_char_p]
        lib.mesmo_address_validate.restype = c_int

        # Crypto API
        lib.mesmo_crypto_blake2b_256.argtypes = [c_void_p, c_char_p]
        lib.mesmo_crypto_blake2b_256.restype = c_int

        lib.mesmo_crypto_blake2b_224.argtypes = [c_void_p, c_char_p]
        lib.mesmo_crypto_blake2b_224.restype = c_int

        lib.mesmo_crypto_generate_mnemonic.argtypes = [c_void_p, c_int]
        lib.mesmo_crypto_generate_mnemonic.restype = c_int

        lib.mesmo_crypto_validate_mnemonic.argtypes = [c_void_p, c_char_p]
        lib.mesmo_crypto_validate_mnemonic.restype = c_int

        lib.mesmo_crypto_sign.argtypes = [c_void_p, c_char_p, c_char_p]
        lib.mesmo_crypto_sign.restype = c_int

        lib.mesmo_crypto_verify.argtypes = [c_void_p, c_char_p, c_char_p, c_char_p]
        lib.mesmo_crypto_verify.restype = c_int

        lib.mesmo_crypto_derive_key.argtypes = [c_void_p, c_char_p, c_int, c_int, c_char_p]
        lib.mesmo_crypto_derive_key.restype = c_int

        # Transaction API
        lib.mesmo_tx_sign_with_secret_key.argtypes = [c_void_p, c_char_p, c_char_p]
        lib.mesmo_tx_sign_with_secret_key.restype = c_int

        lib.mesmo_tx_hash.argtypes = [c_void_p, c_char_p]
        lib.mesmo_tx_hash.restype = c_int

        lib.mesmo_tx_to_json.argtypes = [c_void_p, c_char_p]
        lib.mesmo_tx_to_json.restype = c_int

        lib.mesmo_tx_from_json.argtypes = [c_void_p, c_char_p]
        lib.mesmo_tx_from_json.restype = c_int

        lib.mesmo_tx_deserialize.argtypes = [c_void_p, c_char_p]
        lib.mesmo_tx_deserialize.restype = c_int

        # Plutus API
        lib.mesmo_plutus_data_hash.argtypes = [c_void_p, c_char_p]
        lib.mesmo_plutus_data_hash.restype = c_int

        lib.mesmo_plutus_data_to_json.argtypes = [c_void_p, c_char_p]
        lib.mesmo_plutus_data_to_json.restype = c_int

        lib.mesmo_plutus_data_from_json.argtypes = [c_void_p, c_char_p]
        lib.mesmo_plutus_data_from_json.restype = c_int

        # Script API
        lib.mesmo_script_native_from_json.argtypes = [c_void_p, c_char_p]
        lib.mesmo_script_native_from_json.restype = c_int

        lib.mesmo_script_hash.argtypes = [c_void_p, c_char_p, c_int]
        lib.mesmo_script_hash.restype = c_int

        # QuickTx API
        lib.mesmo_quicktx_build.argtypes = [c_void_p, c_char_p, c_char_p, c_char_p, c_char_p, c_int]
        lib.mesmo_quicktx_build.restype = c_int

        # Managed account handles (ADR-0016)
        lib.mesmo_account_open_mnemonic.argtypes = [c_void_p, c_int, c_char_p, c_int, c_int,
                                                  POINTER(ctypes.c_int64)]
        lib.mesmo_account_open_mnemonic.restype = c_int
        lib.mesmo_account_get_info.argtypes = [c_void_p, ctypes.c_int64]
        lib.mesmo_account_get_info.restype = c_int
        lib.mesmo_account_sign_tx_handle.argtypes = [c_void_p, ctypes.c_int64, c_char_p, c_int]
        lib.mesmo_account_sign_tx_handle.restype = c_int
        lib.mesmo_account_close.argtypes = [c_void_p, ctypes.c_int64]
        lib.mesmo_account_close.restype = c_int
        lib.mesmo_account_create_handle.argtypes = [c_void_p, c_int, POINTER(ctypes.c_int64)]
        lib.mesmo_account_create_handle.restype = c_int
        lib.mesmo_account_export_recovery_phrase.argtypes = [c_void_p, ctypes.c_int64, POINTER(c_void_p)]
        lib.mesmo_account_export_recovery_phrase.restype = c_int
    ...
    def _thread(self):
        ...
        if thread is None:
            ...
            if rc != 0:
                raise MesmoError(rc, f"failed to attach thread to the GraalVM isolate (code {rc})")
            self._local.thread = thread
        return thread

    def _get_result(self):
        """Get the last result string and free it."""
        ptr = self._lib.mesmo_get_result(self._thread)
        if not ptr:
            return None
        result = ctypes.string_at(ptr).decode('utf-8')
        self._lib.mesmo_free_string(self._thread, ptr)
        return result

    def _get_error(self):
        """Get the last error string and free it."""
        ptr = self._lib.mesmo_get_last_error(self._thread)
        if not ptr:
            return None
    ...
    def _check(self, rc):
        ...
        if rc != self.MESMO_SUCCESS:
            error = self._get_error()
            if rc == self.MESMO_ERROR_INVALID_HANDLE:
                raise CclInvalidHandleError(rc, error or f"Unknown error (code {rc})")
            raise MesmoError(rc, error or f"Unknown error (code {rc})")
    ...
    def close(self):
        """Tear down the isolate. Idempotent; further calls raise MesmoClosedError.
    ...
    def version(self):
        rc = self._lib.mesmo_version(self._thread)
        return self._check(rc)

    def _check_version(self):
        """Fail fast on a native-lib / wrapper version skew (bypass with MESMO_SKIP_VERSION_CHECK)."""
        if os.environ.get("MESMO_SKIP_VERSION_CHECK"):
            return
        lib_ver = self.version()
        if _base_version(lib_ver) != _base_version(EXPECTED_LIB_VERSION):
            raise RuntimeError(
                f"libmesmo version {lib_ver!r} is incompatible with the cardano-client-lib Python "
                f"wrapper (expects {EXPECTED_LIB_VERSION!r}). The native library and wrapper must be "
                f"the same version — reinstall the package, or set MESMO_LIB_PATH to a matching libmesmo. "
                f"Set MESMO_SKIP_VERSION_CHECK=1 to bypass."
            )
