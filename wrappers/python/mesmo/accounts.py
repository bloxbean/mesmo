...
class Account:
    ...
    after close raises :class:`mesmo.CclInvalidHandleError`. Garbage collection closes as a fallback
    ...
    def info(self):
        ...
        if self._info is None:
            rc = self._b._lib.mesmo_account_get_info(self._b._thread, self._handle)
    ...
    def sign_tx(self, tx_cbor_hex, roles=SigningRole.PAYMENT):
        ...
        """
        rc = self._b._lib.mesmo_account_sign_tx_handle(
    ...
    def export_recovery_phrase(self):
        ...
        out = ctypes.c_void_p()
        rc = self._b._lib.mesmo_account_export_recovery_phrase(
            self._b._thread, self._handle, ctypes.byref(out))
        if rc != self._b.MESMO_SUCCESS:
            from mesmo._ffi import MesmoError, CclInvalidHandleError
            error = self._b._get_error()
            if rc == self._b.MESMO_ERROR_INVALID_HANDLE:
    ...
    def close(self):
        ...
        if handle and not getattr(self._b, "_closed", True):
            ...
            # its result fetch, on this same thread. mesmo_account_close produces no
...
class Accounts:
    ...
    def from_mnemonic(self, mnemonic, network, account_index=0, address_index=0):
        ...
        rc = self._b._lib.mesmo_account_open_mnemonic(
    ...
    def create(self, network):
        ...
        """
        handle = ctypes.c_int64(0)
        rc = self._b._lib.mesmo_account_create_handle(
