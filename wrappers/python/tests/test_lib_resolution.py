"""Unit tests for how Mesmo locates the native library.
...
def test_explicit_lib_path_wins(monkeypatch):
    ...
    name = Mesmo._lib_filename()
    assert Mesmo._resolve_lib_file("/opt/dir") == os.path.join("/opt/dir", name)
...
def test_env_var_used_when_no_explicit_path(monkeypatch):
    ...
    name = Mesmo._lib_filename()
    assert Mesmo._resolve_lib_file() == os.path.join("/env/dir", name)
...
def test_bundled_lib_preferred_when_no_env(monkeypatch):
    ...
    resolved = Mesmo._resolve_lib_file()
    assert resolved.endswith(os.path.join("_libs", Mesmo._lib_filename()))


def test_bare_filename_fallback(monkeypatch):
    monkeypatch.delenv("MESMO_LIB_PATH", raising=False)
    monkeypatch.setattr(os.path, "exists", lambda p: False)  # nothing bundled
    assert Mesmo._resolve_lib_file() == Mesmo._lib_filename()
