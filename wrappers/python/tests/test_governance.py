...
def test_gov_identifiers_in_account_info(mesmo):
...
def test_derive_key_matches_account_credentials(mesmo):
    """The stateless derivation utility and the handle agree on the committee credentials."""
    with mesmo.accounts.create(Network.MAINNET) as acct:
        info = acct.info
        mnemonic = acct.export_recovery_phrase()

    cold = mesmo.crypto.derive_key(mnemonic, role="committee_cold")
    hot = mesmo.crypto.derive_key(mnemonic, role="committee_hot")
...
def test_derive_key_shapes(mesmo):
    mnemonic = mesmo.crypto.generate_mnemonic(24)
    drep = mesmo.crypto.derive_key(mnemonic, role="drep")
...
def test_derive_key_returns_cip105_bech32_encodings_for_gov_roles(mesmo):
    ...
    for role, prefix in (("drep", "drep"), ("committee_cold", "cc_cold"), ("committee_hot", "cc_hot")):
        key = mesmo.crypto.derive_key(mnemonic, role=role)
    ...
    payment = mesmo.crypto.derive_key(mnemonic, role="payment")
