...
def test_wallet_restore_shares_stake_identity(mesmo):
...
def test_wallet_address_enumeration(mesmo):
    with mesmo.accounts.create(Network.MAINNET) as created:
        mnemonic = created.export_recovery_phrase()

    addresses = []
    for index in range(2):
        with mesmo.accounts.from_mnemonic(mnemonic, Network.MAINNET, 0, index) as acct:
...
def test_wallet_create_testnet(mesmo):
