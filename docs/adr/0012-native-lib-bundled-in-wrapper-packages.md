...
Mesmo ships a native shared library (`libmesmo.{so,dylib,dll}`, [ADR-0001](0001-native-shared-library-ffi.md))
that each language wrapper ([ADR-0003](0003-four-language-wrappers-uniform-ffi.md)) loads over FFI. Until
now the wrappers only located that library through an environment variable (`MESMO_LIB_PATH`, plus the OS
...
normal install needs no environment variables. `MESMO_LIB_PATH` remains supported as a **development / override
...
- **Python — platform wheels (implemented).** A `py3-none-<platform>` wheel bundles the matching `libmesmo.*`
  under `mesmo/_libs/`. Pure-`ctypes` binding, so one wheel works on any Python 3 for that platform; only the
...
- **Rust — crates.io (implemented).** crates.io won't host the ~50 MB binary, so the crate carries only
  source + `build.rs`; `build.rs` sources `libmesmo.*` (from `MESMO_LIB_PATH`, the in-tree build, or the GitHub
  release), stages a copy in `OUT_DIR`, rewrites the macOS install name to `@rpath`, and emits an `rpath` —
  so linking *and* runtime work with no env vars. CI proves the crate loads with `MESMO_LIB_PATH`/`DYLD`/`LD`
...
- **The two wrapper families resolve the lib differently.** Python (ctypes) and JS (`dlopen`) load a file
  ...
  **native-linked** wrappers (Rust `extern "C"` and C) *link* against `libmesmo` at build time — Go resolves it at runtime via purego ([ADR-0014](0014-go-distribution-purego-runtime-resolution.md)) —
  so removing the env-var requirement means making the runtime loader find it: stage the lib and reference
  it via **`@rpath`** (macOS needs `install_name_tool -id @rpath/libmesmo.dylib` because GraalVM stamps an
...
- **Keep `MESMO_LIB_PATH`-only (status quo).** Zero packaging work, but leaves the primary adoption blocker in
...
- **Runtime `dlopen` for the native-linked wrappers, instead of build-time linking + `@rpath`.** Rust/C/Go
  *could* drop `extern "C"`/cgo and load `libmesmo` at runtime (via `libloading` / `purego`, like Python and
  ...
  **purego** and resolves `libmesmo` at runtime (`MESMO_LIB_PATH` → cache → one-time download) — not
