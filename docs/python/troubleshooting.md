...
3. A copy bundled inside the installed wheel (`mesmo/_libs/`) — this is what a normal `pip install` uses.
...
- **Wheel install:** make sure you installed a wheel built for your platform (check `pip show -f cardano-client-lib` lists `mesmo/_libs/libmesmo...`). A source install has no bundled library — set `MESMO_LIB_PATH`.
...
> Alpine/musl: the musl native library exists (the Go/Rust/JS wrappers use it), but musllinux wheel publishing is still being wired up — on Alpine, install from source with `MESMO_LIB_PATH` pointing at the musl `libmesmo.so` for now.
