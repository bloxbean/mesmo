...
- **More chain-data providers.** The initial version ships `ChainDataProvider` implementations for Yaci-Store and Blockfrost; additional out-of-the-box providers are planned — **Ogmios**, **Koios**, and **Dolos** — so UTXO selection works against whichever data backend your infrastructure already runs. None of this is gated on a release, though: the interface is two methods (fetch UTXOs, fetch protocol parameters), so writing your own — a `DolosProvider`, say — is possible today in a few dozen lines.
