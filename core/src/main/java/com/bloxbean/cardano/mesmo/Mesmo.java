...
/**
 * Core lifecycle and result-retrieval entry points of the Mesmo native library.
 *
 * <h2>What this library is</h2>
 * The Mesmo compiles <a href="https://github.com/bloxbean/cardano-client-lib">Cardano
 * Client Lib</a> into a native shared library ({@code libmesmo.so} / {@code .dylib} / {@code .dll})
 ...
 *       {@link #getResult(IsolateThread) mesmo_get_result}. Results are JSON strings (or a bare
 ...
 *       {@link #getLastError(IsolateThread) mesmo_get_last_error}.</li>
 *   <li><b>Returned strings</b> are allocated in unmanaged (malloc'd) memory and <em>must</em> be
 *       released by the caller with {@link #freeString(IsolateThread, CCharPointer) mesmo_free_string}.</li>
 * </ul>
 *
 * <h2>Typical sequence (per logical operation)</h2>
 * <pre>{@code
 *   int rc = mesmo_address_info(thread, address);        // 1. do the work; returns status
 *   if (rc == 0) {
 *       char* json = mesmo_get_result(thread);           // 2. fetch JSON result (thread-local)
 *       // ... use json ...
 *       mesmo_free_string(thread, json);                 // 3. release the malloc'd string
 *   } else {
 *       char* err = mesmo_get_last_error(thread);        // or fetch the error message
 *       mesmo_free_string(thread, err);
 *   }
 * }</pre>
 *
 * <p>The result slot is <b>read-once</b>: {@code mesmo_get_result} consumes the value, and a second
 ...
 *
 * <p>Because the result/error are thread-local, the work call and its {@code mesmo_get_result} /
 * {@code mesmo_get_last_error} retrieval must run on the same isolate thread.
 *
 * @see ErrorCodes status codes returned by every entry point
 */
public final class Mesmo {
    ...
    /**
     * Returns the Mesmo library version.
     *
     * <p>Exported as {@code mesmo_version}. On success the version string (e.g. {@code "0.1.0"}) is
     * placed in the thread-local result; retrieve it with
     * {@link #getResult(IsolateThread) mesmo_get_result}.
     *
     * @param thread the current isolate thread
     * @return {@link ErrorCodes#MESMO_SUCCESS}
     */
    @CEntryPoint(name = "mesmo_version")
     ...
     *
     * <p>Exported as {@code mesmo_get_result}. The returned pointer is malloc'd and owned by the
     * caller, who must release it with {@link #freeString(IsolateThread, CCharPointer) mesmo_free_string}.
     ...
     */
    @CEntryPoint(name = "mesmo_get_result")
     ...
     *
     * <p>Exported as {@code mesmo_get_last_error}. Call this after an entry point returns a negative
     * {@link ErrorCodes status code}. The returned pointer is malloc'd and owned by the caller, who
     * must release it with {@link #freeString(IsolateThread, CCharPointer) mesmo_free_string}. If no
     ...
     */
    @CEntryPoint(name = "mesmo_get_last_error")
     ...
     *
     * <p>Exported as {@code mesmo_free_string}. Every non-{@code NULL} pointer returned by
     * {@code mesmo_get_result} / {@code mesmo_get_last_error} is allocated in unmanaged memory and
     ...
     */
    @CEntryPoint(name = "mesmo_free_string")
