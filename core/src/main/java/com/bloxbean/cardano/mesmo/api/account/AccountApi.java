package com.bloxbean.cardano.mesmo.api.account;
 ...
 * {@link ErrorCodes#MESMO_ERROR_INVALID_HANDLE} — foreign-handle detection is statistical
 * (collision odds ~2^-62), not structural. Closing is explicit and idempotent; all handles die
 * with the isolate.
 *
 * <p>See {@link com.bloxbean.cardano.mesmo.Mesmo} for the calling convention.
 */
public final class AccountApi {
     ...
     *
     * <p>Exported as {@code mesmo_account_open_mnemonic}. On success writes the handle to
     * {@code out_handle} and returns {@link ErrorCodes#MESMO_SUCCESS}. The account's network and
     * derivation path are fixed for the handle's lifetime; no result string is produced (fetch
     * public data with {@code mesmo_account_get_info}).
     *
     ...
     *         {@link ErrorCodes#MESMO_ERROR_INVALID_NETWORK} /
     *         {@link ErrorCodes#MESMO_ERROR_INVALID_MNEMONIC} / {@link ErrorCodes#MESMO_ERROR_GENERAL}
     */
    @CEntryPoint(name = "mesmo_account_open_mnemonic")
    public static int openMnemonic(IsolateThread thread, int networkId, CCharPointer mnemonicPtr,
        ...
        try {
            ...
            return ErrorCodes.MESMO_ERROR_INVALID_NETWORK;
            ...
            ErrorState.set(e.getMessage());
            return ErrorCodes.MESMO_ERROR_GENERAL;
        }
    }
     ...
     *
     * <p>Exported as {@code mesmo_account_create_handle}. On success writes the handle to
     * {@code out_handle}; <b>no secret is returned</b> — the recovery phrase is retrievable once,
     * deliberately, via {@code mesmo_account_export_recovery_phrase}.
     *
     * @param thread    the current isolate thread
     * @param networkId 0=mainnet, 1=testnet
     * @param outHandle receives the opaque account handle (must be non-null)
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_INVALID_NETWORK} /
     *         {@link ErrorCodes#MESMO_ERROR_INVALID_ARGUMENT} / {@link ErrorCodes#MESMO_ERROR_GENERAL}
     */
    @CEntryPoint(name = "mesmo_account_create_handle")
     ...
     *
     * <p>Exported as {@code mesmo_account_export_recovery_phrase}. On success {@code out_phrase}
     * receives a malloc'd, NUL-terminated copy of the BIP-39 phrase (free it with
     * {@code mesmo_free_string}). The pending phrase is destroyed only <em>after</em> that copy has
     ...
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_INVALID_HANDLE} /
     *         {@link ErrorCodes#MESMO_ERROR_INVALID_ARGUMENT}
     */
    @CEntryPoint(name = "mesmo_account_export_recovery_phrase")
    public static int exportRecoveryPhrase(IsolateThread thread, long handle,
        ...
        try {
            ...
            try {
                outPhrase.write(NativeString.toCString(phrase)); // caller frees via mesmo_free_string
            } catch (Throwable t) {
            ...
            ErrorState.set(e.getMessage());
            return ErrorCodes.MESMO_ERROR_INVALID_HANDLE;
     ...
     *
     * <p>Exported as {@code mesmo_account_get_info}. On success the result is a JSON object:
     ...
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_INVALID_HANDLE} /
     *         {@link ErrorCodes#MESMO_ERROR_GENERAL}
     */
    @CEntryPoint(name = "mesmo_account_get_info")
     ...
     *
     * <p>Exported as {@code mesmo_account_sign_tx_handle}. {@code role_mask} bits: {@code 1}=payment,
     ...
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_INVALID_HANDLE} /
     *         {@link ErrorCodes#MESMO_ERROR_INVALID_ARGUMENT} /
     *         {@link ErrorCodes#MESMO_ERROR_INVALID_TRANSACTION}
     */
    @CEntryPoint(name = "mesmo_account_sign_tx_handle")
    public static int signTx(IsolateThread thread, long handle, CCharPointer txCborPtr, int roleMask) {
        try {
            ...
            ErrorState.set(e.getMessage());
            return ErrorCodes.MESMO_ERROR_INVALID_TRANSACTION;
        }
    }
     ...
     *
     * <p>Exported as {@code mesmo_account_close}. Idempotent: closing an unknown or already-closed
     * handle succeeds (returns {@link ErrorCodes#MESMO_SUCCESS}) — double-close must be safe for
     * wrapper finalizers.
     *
     * @param thread the current isolate thread
     * @param handle the account handle to close
     * @return {@link ErrorCodes#MESMO_SUCCESS}
     */
    @CEntryPoint(name = "mesmo_account_close")
