 ...
 * {@link com.bloxbean.cardano.mesmo.Mesmo} for the calling convention. Every entry point here
...
public final class TransactionApi {
     ...
     *
     * <p>Exported as {@code mesmo_tx_sign_with_secret_key}. On success the result is the signed
     * transaction as CBOR hex. Use this when you hold a key directly rather than a managed
     * account handle (cf. {@code mesmo_account_sign_tx_handle}).
     *
     ...
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_INVALID_TRANSACTION}
     */
    @CEntryPoint(name = "mesmo_tx_sign_with_secret_key")
     ...
     *
     * <p>Exported as {@code mesmo_tx_hash}. On success the result is the 32-byte hash as hex (64 hex
     ...
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_INVALID_TRANSACTION}
     */
    @CEntryPoint(name = "mesmo_tx_hash")
     ...
     *
     * <p>Exported as {@code mesmo_tx_to_json}. On success the result is the transaction as JSON (for
     * inspection — this is not the on-chain format). Equivalent to {@code mesmo_tx_deserialize}.
     *
     * @param thread       the current isolate thread
     * @param txCborHexPtr the transaction as CBOR hex
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_SERIALIZATION}
     */
    @CEntryPoint(name = "mesmo_tx_to_json")
    public static int toJson(IsolateThread thread, CCharPointer txCborHexPtr) {
        try {
            ...
            Transaction tx = Transaction.deserialize(HexUtil.decodeHexString(txCborHex));
            String json = com.bloxbean.cardano.mesmo.util.JsonHelper.toJson(tx);
     ...
     *
     * <p>Exported as {@code mesmo_tx_from_json}. The inverse of {@code mesmo_tx_to_json}: on success the
     ...
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_SERIALIZATION}
     */
    @CEntryPoint(name = "mesmo_tx_from_json")
    public static int fromJson(IsolateThread thread, CCharPointer txJsonPtr) {
        try {
            ...
            Transaction tx = com.bloxbean.cardano.mesmo.util.JsonHelper.fromJson(txJson, Transaction.class);
     ...
     *
     * <p>Exported as {@code mesmo_tx_deserialize}. On success the result is the transaction as JSON
     * (for inspection). Same output as {@code mesmo_tx_to_json}.
     *
     * @param thread       the current isolate thread
     * @param txCborHexPtr the transaction as CBOR hex
     * @return {@link ErrorCodes#MESMO_SUCCESS}, or {@link ErrorCodes#MESMO_ERROR_SERIALIZATION}
     */
    @CEntryPoint(name = "mesmo_tx_deserialize")
